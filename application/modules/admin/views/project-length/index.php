  ...
  <div class="page-title">
    <div style="width: 100%">
      <h1 class="text-uppercase"> 
          Project Length
          <button data-toggle="modal" data-target="#modalAddData" class="btn btn-sm btn-primary pull-right"> ADD ROJECT LENGTH</button>
      </h1>
    </div>
  </div>
  <div class="row">
                 ...
                 <div class="table-responsive">
                   <table class="table">
                    ...
                    if(!empty($project_lengths)){
                      $count = 0;
                      foreach ($project_lengths as $data) {
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Add PROJECT LENGTH</h4>
      </div>
...
<div id="modalEditData" class="modal fade" role="dialog">
  <div class="modal-dialog"> 
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Edit PROJECT LENGTH</h4>
      </div>
      <div class="modal-body">
         <form method="POST" id="FormEditData" name="FormEditData" enctype="multipart/form-data">
            <div class="form-group">
              ...
              <div class="text-center">
                <button type="submit" class="btn btn-primary text-uppercase">SAVE PROJECT LENGTH</button>
              </div>
          </form>
      </div>
    </div>
 ...
 <script type="text/javascript"> 
    $('#FormAddData').on('submit', function(e){       
        ...
        // $('#myloader').show();
        $.ajax({
            url: '<?php echo base_url('admin/add-project-length'); ?>',
            type: "POST",
    ...
    $('#FormEditData').on('submit', function(e){       
        ...
        // $('#myloader').show();
        $.ajax({
            url: '<?php echo base_url('admin/save-project-length'); ?>',
            type: "POST",
            data: new FormData(this),
            contentType: false,
            cache: false,
    ...
    $('#FormDeleteData').on('submit', function(e){       
      ...
      // $('#myloader').show();
      $.ajax({
          url: '<?php echo base_url('admin/delete-project-length'); ?>',
          type: "POST",
          data: new FormData(this),
          contentType: false,
          cache: false,
