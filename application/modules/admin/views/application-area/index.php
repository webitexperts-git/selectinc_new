  ...
  <div class="page-title">
    <div style="width: 100%">
      <h1 class="text-uppercase"> 
          Application Area 
          <button data-toggle="modal" data-target="#modalAddData" class="btn btn-sm btn-primary pull-right"> ADD APP AREA</button>
      </h1>
    </div>
  </div>
  <div class="row">
                   ...
                   <table class="table">
                    ...
                    ?>
                      <tr>
                        ...
                        <td> <?php echo strip_tags($app_area->name)  ?>  </td>

                        <td> <button  type='button' class='btn btn-primary btn-sm' data-toggle='modal' data-target='#modalEditData' 
                          data-id="<?php echo $app_area->id; ?>"  
                          data-title="<?php echo base64_encode($app_area->name); ?>"  
                          onclick="
                          $('#txeditid').val($(this).attr('data-id')); 
                          $('#txeditttitle').val(atob($(this).attr('data-title'))); 
                          "><i class="fa fa-pencil-square-o"></i> EDIT</button> </td>
                        <td> <button  type='button' class='btn btn-danger btn-sm' data-toggle='modal' data-target='#modalDeleteData' data-id="<?php echo $app_area->id; ?>"  onclick="$('#txtdelid').val($(this).attr('data-id'))"  ><i class="fa fa-trash"></i> DELETE</button></td>
                      </tr>
                    <?php
                      }
                    }else{
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Add Application Area</h4>
      </div>
...
<div id="modalEditData" class="modal fade" role="dialog">
  <div class="modal-dialog"> 
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Edit Application Area</h4>
      </div>
      <div class="modal-body">
         <form method="POST" id="FormEditData" name="FormEditData" enctype="multipart/form-data">
            <div class="form-group">
              ...
              <div class="text-center">
                <button type="submit" class="btn btn-primary text-uppercase">SAVE Application Area</button>
              </div>
          </form>
      </div>
    </div>
 ...
 <script type="text/javascript"> 
    $('#FormAddData').on('submit', function(e){       
        ...
        // $('#myloader').show();
        $.ajax({
            url: '<?php echo base_url('admin/add-application-area'); ?>',
            type: "POST",
    ...
    $('#FormEditData').on('submit', function(e){       
        ...
        // $('#myloader').show();
        $.ajax({
            url: '<?php echo base_url('admin/save-application-area'); ?>',
            type: "POST",
            data: new FormData(this),
            contentType: false,
            cache: false,
    ...
    $('#FormDeleteData').on('submit', function(e){       
      ...
      // $('#myloader').show();
      $.ajax({
          url: '<?php echo base_url('admin/delete-application-area'); ?>',
          type: "POST",
          data: new FormData(this),
          contentType: false,
          cache: false,
