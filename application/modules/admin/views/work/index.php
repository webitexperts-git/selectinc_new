  ...
  <div class="page-title">
    <div style="width: 100%">
      <h1 class="text-uppercase"> 
         Work Type
          <button data-toggle="modal" data-target="#modalAddData" class="btn btn-sm btn-primary pull-right"> ADD WORK TYPE </button>
      </h1>
    </div>
  </div>
  <div class="row">
...
<div id="modalEditData" class="modal fade" role="dialog">
      ...
      <div class="modal-body">
         <form method="POST" id="FormEditData" name="FormEditData" enctype="multipart/form-data">
            <div class="form-group">
              ...
              <div class="text-center">
                <button type="submit" class="btn btn-primary text-uppercase">SAVE WORK TYPE</button>
              </div>
          </form>
      </div>
    </div>
 ...
 <script type="text/javascript"> 
    $('#FormAddData').on('submit', function(e){       
        ...
        // $('#myloader').show();
        $.ajax({
            url: '<?php echo base_url('admin/add-work'); ?>',
            type: "POST",
    ...
    $('#FormEditData').on('submit', function(e){       
        ...
        // $('#myloader').show();
        $.ajax({
            url: '<?php echo base_url('admin/save-work'); ?>',
            type: "POST",
            data: new FormData(this),
            contentType: false,
            cache: false,
    ...
    $('#FormDeleteData').on('submit', function(e){       
      ...
      // $('#myloader').show();
      $.ajax({
          url: '<?php echo base_url('admin/delete-work'); ?>',
          type: "POST",
          data: new FormData(this),
          contentType: false,
          cache: false,
